...
function getChannelId() {
    ...
    if (authorSpan) {
        const linkElement = authorSpan.querySelector('link[itemprop="url"]');
...
function showPopup() {
    if (document.getElementById("youtube-extension-popup"))
        return;
    const popup = document.createElement("div");
  ...
  `;
    ...
    [likeCheckbox, commentCheckbox, subscribeCheckbox].forEach((checkbox) => checkbox.addEventListener("change", updateExecuteButtonState));
    function updateExecuteButtonState() {
        executeBtn.disabled = !(likeCheckbox.checked ||
...
function executeSelectedActions(commentText) {
    const likeChecked = document.getElementById("like-checkbox").checked;
    const commentChecked = document.getElementById("comment-checkbox").checked;
        ...
        chrome.storage.local.get("accessToken", (data) => {
            if (data.accessToken) {
                chrome.runtime.sendMessage({
                    ...
                    if (response.success) {
                        alert("Comment posted successfully! To see the comment, please refresh your browser and if you still don't see the comment, please wait for some time as youtube apis take some time to reflect.");
                    }
                    else {
                        console.error("Failed to post comment:", response.error);
                        alert(`Failed to post comment. ${response === null || response === void 0 ? void 0 : response.error} `);
                    }
                });
            }
            else {
    ...
    chrome.storage.local.get("accessToken", (data) => {
        if (data.accessToken) {
            chrome.runtime.sendMessage({
                ...
                if (response.success) {
                    alert(`Video liked successfully! To see liked video, please refresh your browser and if you still don't see the video liked, please wait for some time as youtube apis take some time to reflect."`);
                }
                else {
    ...
    chrome.storage.local.get("accessToken", (data) => {
        if (data.accessToken) {
            chrome.runtime.sendMessage({
                ...
                if (response.success) {
                    alert("Successfully subscribed to the channel! To see the subscribed status, please refresh your browser and if you still don't see channel subscribed, please wait for some time as youtube apis take some time to reflect.");
                }
                else {
                    alert(`Failed to subscribe. ${response === null || response === void 0 ? void 0 : response.error} `);
                }
            });
        }
        else {
